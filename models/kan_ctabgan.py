...
from torch.nn import (Dropout, LeakyReLU, Linear, Module, ReLU, Sequential, SiLU,
...
class KANClassifier(Module):
    def __init__(self, input_dim, dis_dims, st_ed, grid=5, k=3):
        ...
        for item in list(dis_dims):
            seq += [KANLinear(dim, item, grid_size=grid, spline_order=k), LeakyReLU(0.2), Dropout(0.5)]
            dim = item
        
        if (st_ed[1]-st_ed[0])==1:
            seq += [KANLinear(dim, 1, grid_size=grid, spline_order=k)]
        
        elif (st_ed[1]-st_ed[0])==2:
            seq += [KANLinear(dim, 1, grid_size=grid, spline_order=k),Sigmoid()]
        else:
            seq += [KANLinear(dim,(st_ed[1]-st_ed[0]), grid_size=grid, spline_order=k)] 
...
class KANCTABGANSynthesizer:
    ...
    def fit(self, train_data=pd.DataFrame, categorical=[], mixed={}, general=[], non_categorical=[], type={}):
        ...
        if target_index != None:
            ...
            classifier = KANClassifier(data_dim,self.class_dim,st_ed).to(self.device)
...
class KANCTABGAN():
    ...
    def __init__(self,
        ...
        self.__name__ = 'KANCTABGAN'
              
        self.synthesizer = KANCTABGANSynthesizer()
        self.raw_df = raw_df
